...
public class BohnanzaController implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
				...
				if (e.getSource() == gameFrame.getCommonPanel().getDiscardButton()) {
					...
					for (int i = 0; i < 3; i++) {
						if (!(gameFrame.getCommonPanel().getSlots()[i].getIcon().toString()
							...
							System.out.println(cardDiscard.getBeanType());
		...
		if (e.getSource() == currentPlayer.getPanel().getField().getActionBtns()[0]
			...
			if (e.getSource() == currentPlayer.getPanel().getField().getActionBtns()[0] && (player.sell(0))) {
				gui.updateSell(currentPlayer, 0);
				JOptionPane.showMessageDialog(gameFrame, currentPlayer.getName() + " Sold Their Beans");
			} else if (e.getSource() == currentPlayer.getPanel().getField().getActionBtns()[1] && (player.sell(1))) {
				gui.updateSell(currentPlayer, 1);
				JOptionPane.showMessageDialog(gameFrame, currentPlayer.getName() + " Sold Their Beans");
			} else {
				JOptionPane.showMessageDialog(gameFrame, "Nothing to sell! Sell a field with beans planted.",
						"Unable To Process Request", JOptionPane.ERROR_MESSAGE);
			}
		}
	...
	private void validateCards(int i) {
		...
		if (identical) {
			identical = false;
			JOptionPane.showMessageDialog(gameFrame, "Identical Cards In Slots Have Been Stacked", ".",
					JOptionPane.ERROR_MESSAGE);
		}
	}
